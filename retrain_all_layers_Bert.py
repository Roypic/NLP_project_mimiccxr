...
from deep_utils import DirUtils, StringUtils
...
# freeze bert
StringUtils.print("Setting bert to freeze :)")
...
for epoch in range(EPOCHS):
    ...
    if val_f1_score > best_f1_score:
        torch.save(model.state_dict(), "50-50/clinicalbert_75x3_best.pt")
        best_f1_score = val_f1_score

    f1_score_values.append(val_f1_score)
    torch.save(model.state_dict(), "50-50/clinicalbert_75x3_last.pt")
...
DirUtils.write_txt("50-50/test_f1_score.txt", [f'{test_f1_score}'], mode="w")
# Save model
...
# finetune bert
StringUtils.print("Setting bert to finetune :)")
for p in model.bert.parameters():
...
for epoch in range(EPOCHS):
    ...
    if val_f1_score > best_f1_score:
        torch.save(model.state_dict(), "50-50/clinicalbert_75x3_finetune_both_best.pt")
        best_f1_score = val_f1_score

    torch.save(model.state_dict(), "50-50/clinicalbert_75x3_finetune_both_last.pt")
    # print("Model saved to clinicalbert_75x3_last.pt")
    print(f"Epoch {epoch + 1}/{EPOCHS}")
    print(f"  Train loss: {train_loss:.4f}")
    print(f"  Val   loss: {val_loss:.4f}")
...
test_loss, test_preds, test_labels = eval_one_epoch(model, test_loader, DEVICE)
test_f1_score = multi_f1_score(test_labels, test_preds)
DirUtils.write_txt("50-50/test_finetune_both_f1_score.txt", [f'{test_f1_score}'], mode="w")
